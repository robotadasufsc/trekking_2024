...
frente = 2
re = 2
duracao = 4 # por exemplo, 5 segundos
Ts = 0.001
...
while time.time() - tempo_inicial < duracao:
    ...
    acceleration = navigator.read_accel()
    accel_x = acceleration.x
    accel_y = calibracao.add_value(valor_filtro, acceleration.y, 100, 1, 20000) - offsety
    accel_z = acceleration.z
...
plt.subplot(3, 1, 2)
plt.plot(range(len(velocity)), velocity, label='m/s')
plt.xlabel('Time')
plt.ylabel('Values')
...
print(f'percorreu {calculate_final_position(acceleration, time_step)} metros')
plt.show()