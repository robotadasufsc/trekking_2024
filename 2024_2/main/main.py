...
# inicia comunicacao entre rasp e arduino
porta = '/dev/ttyACM0'
freq = 9600
ser = funcao_sonar3.configurar_porta(porta, freq)

# define onde o servo ira comecar
lado = "meio"
...
# funcao que para o motor
def stop_motor():
    init()
    gpio.output(IN1_PIN_A, gpio.LOW)
    gpio.output(IN2_PIN_A, gpio.LOW)

# funcao que faz o robo ir pra tras
def move_backward(): 
    init()
    gpio.output(IN1_PIN_A, gpio.LOW)
...
while True:
    ...
    if 200 < valor[0] < 1000 and lado != "direita": 
        ...
        lt()
    elif 200 > valor[0] > 20 and lado != "meio":
        mid()